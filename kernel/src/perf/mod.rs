...
mod uprobe;
use alloc::{
    borrow::Cow, boxed::Box, sync::{Arc, Weak}
};
...
use crate::{
    bpf::tansform::EbpfKernelAuxiliary, file::{FileLike, Kstat, add_file_like, get_file_like}, mm::AddrSpace, perf::bpf::BpfPerfEventWrapper
};
...
impl FileLike for PerfEvent {
    fn read(&self, _dst: &mut crate::file::IoDst) -> AxResult<usize> {
        unimplemented!("PerfEvent::read() is not implemented");
    }

    fn write(&self, _src: &mut crate::file::IoSrc) -> AxResult<usize> {
        unimplemented!("PerfEvent::write() is not implemented");
    }
...
pub fn perf_event_output(_ctx: *mut c_void, fd: usize, _flags: u32, data: &[u8]) -> AxResult<()> {
    ...
    let bpf_event_file = file.into_any_arc().downcast::<PerfEvent>().unwrap();
