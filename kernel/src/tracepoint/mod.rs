...
use axtask::current;
use ktracepoint::{KernelTraceOps, TraceEntryParser, TracePipeOps, TracingEventsManager};
...
use crate::{
    ...
    pseudofs::{DirMaker, DirMapping, SimpleDir, SimpleFile, SimpleFs, debug::DebugFsFile},
    task::AsThread,
};
...
mod tests {
    ...
    define_static_key_false_generic!(
        MY_STATIC_KEY,
        ktracepoint::KernelCodeManipulator<KernelTraceAux>
    );
    #[inline(always)]
    fn foo() {
...
static TRACE_RAW_PIPE: KSpinNoPreempt<ktracepoint::TracePipeRaw> =
    KSpinNoPreempt::new(ktracepoint::TracePipeRaw::new(4096));

static TRACE_CMDLINE_CACHE: KSpinNoPreempt<ktracepoint::TraceCmdLineCache> =
    KSpinNoPreempt::new(ktracepoint::TraceCmdLineCache::new(128));
...
impl KernelTraceOps for KernelTraceAux {
    ...
    fn write_kernel_text(addr: *mut core::ffi::c_void, data: &[u8]) {
        ...
        unsafe {
            ...
            axhal::asm::flush_tlb(Some(virt_addr));
        }
        kspace
            .lock()
            .protect(
...
pub fn tracepoint_init() {
    ...
    let entries = static_keys::jump_entries_count();
    ax_println!("tracepoint_init: static keys jump entries count: {}", entries);
    static_keys::global_init();
    TRACE_POINT_MANAGER.call_once(|| {
        ktracepoint::global_init_events::<KSpinNoPreempt<()>, KernelTraceAux>()
