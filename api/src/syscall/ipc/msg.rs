...
use crate::syscall::{sys_getgid, sys_getuid};
...
pub fn sys_msgsnd(msqid: i32, msgp: usize, msgsz: usize, msgflg: i32) -> AxResult<isize> {
    ...
    let mtype: i64 = (msgp as *const i64).vm_read()?;
    ...
    // read data part
    let mut data_buffer = vec![MaybeUninit::<u8>::uninit(); msgsz];
    vm_read_slice(data_ptr as *const u8, &mut data_buffer)?;

    let data_slice = unsafe {
        core::slice::from_raw_parts(data_buffer.as_ptr() as *const u8, data_buffer.len())
    };
...
pub fn sys_msgrcv(
    ...
    // Write mtype
    let mtype_ptr = msgp as *mut i64;
    mtype_ptr.vm_write(mtype)?;

    // Write data part
    let data_ptr = msgp + core::mem::size_of::<i64>();
    let copy_len = data.len().min(msgsz);
    vm_write_slice(data_ptr as *mut u8, &data[..copy_len])?;
...
pub fn sys_msgctl(msqid: i32, cmd: i32, buf: usize) -> AxResult<isize> {
    ...
    if cmd == MSG_INFO {
        ...
        // Copy to user space
        let ptr = buf as *mut starry_core::msg::MsqidDs;
    ...
    if cmd == IPC_STAT {
        ...
        let ptr = buf as *mut starry_core::msg::MsqidDs;
        ptr.vm_write(msg_inner.msqid_ds)?;
    ...
    if cmd == IPC_SET {
        ...
        let ptr = buf as *const starry_core::msg::MsqidDs;
        let uninit = ptr.vm_read_uninit()?; // return MaybeUninit<MsqidDs>
        let user_buf = unsafe { uninit.assume_init() };
